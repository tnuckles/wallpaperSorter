...
import os, shutil, math, datetime, time, json, glob, pikepdf, copy
...
def BatchOrdersMain():
    ...
    try:
        ...
        print('\n| Please enter a wholenumber, not text.')
...
def newBatchDir(material):
    batchDir = gv.batchFoldersDir + 'Batch #' + str(gv.globalBatchCounter['batchCounter']) + ' ' + material + ' (In Progress)'
    gv.globalBatchCounter['batchCounter'] += 1
    os.mkdir(batchDir)
    print('| New Batch Folder:', batchDir.split('/')[-1])
    return batchDir